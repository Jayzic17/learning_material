...
#   4. Run: python3 wotd.py [single, each, grammar] [number]
...
#       * [grammar] [number] randomly returns the given number of grammar points
...
elif str(sys.argv[1]) == 'grammar' and len(sys.argv) != 3:
    sys.exit("Invalid arguments for 'grammar': python3 wotd.py [single, each, grammar] [number]")
...
if str(sys.argv[1]) == 'grammar' and len(sys.argv) == 3:

    grammar_points = []

    # Read from Japanese__Export__3.txt file
    j_exp_file = open('Japanese__Export__3.txt', 'r')
